...
import { getAllUsers, deleteUser, clearUserError } from '../../features/user/userSlice';
...
const UsersList = () => {
    ...
    const itemsToDisplay = allUsers.slice(startIndex, endIndex);
  ...
  return (
    isLoadingUser ? <div className="w-screen min-h-screen flex items-center justify-center"><Loader/></div>:
    <>
        ...
        <div className="flex flex-col md:flex-row">
            ...
            <h1 className='text-2xl text-center font-poppins font-medium pl-5 py-5'>All Users</h1>
                    ...
                    itemsToDisplay && itemsToDisplay?.map((item)=>(
                        <div className="w-full flex flex-col gap-2 border border-slate-500 rounded-lg p-2 my-4" key={item?._id}>
                            <div className="w-full text-xs font-poppins flex items-center justify-between">
                                <p className='text-sm font-medium font-roboto'>User ID</p>
                                <p>{item?._id}</p>
                            </div>
                            <div className="w-full text-xs font-poppins flex items-center justify-between gap-10">
                                <p className='whitespace-nowrap text-sm font-medium font-roboto'>Email</p>
                                <p className={`line-clamp-1 truncate overflow-hidden`}>{item?.email}</p>
                            </div>
                            <div className="w-full md:px-5 flex  justify-between">
                                <div className="text-xs font-poppins flex flex-col items-center justify-between">
                                    <p className='text-sm font-medium font-roboto'>Name</p>
                                    <p>{item?.name}</p>
                                </div>
                                <div className="text-xs font-poppins flex flex-col items-center justify-between">
                                    <p className='text-sm font-medium font-roboto'>Role</p>
                                    <p className='font-roboto'>{item?.role}</p>
                                </div>
                                <div className=" text-xs font-poppins flex flex-col items-center justify-between">
                                    <p className='text-sm font-medium font-roboto'>Actions</p>
                                    <div className="flex gap-2">
                                    <Link to={`/admin/user/${item?._id}`}><span className='text-lg text-green-600 cursor-pointer'><MdEdit/></span></Link> 
                                        <span className='text-lg text-red-600 cursor-pointer' onClick={()=>deleteUserHandler(item?._id)} ><MdDelete/></span>
                                    </div>
                                </div>
                            </div>
                        </div>
               ...
               <div className="">
                {
                    itemsToDisplay && itemsToDisplay?.map((item)=>(
                        <div className="my-5 font-poppins flex items-center justify-between gap-6 border border-slate-400 p-3 hover:bg-gray-400 hover:text-white transition-colors duration-300 ease-in rounded-md">
                            ...
                            <p className={`text-[12px] font-poppins  truncate overflow-hidden `}>{item?.email}</p>
                            <p className=' text-[12px] text-center'>{item?.name}</p> 
                            <p className=' text-[12px] text-center font-roboto'>{item?.role}</p>
                            <div className="flex items-center">
                                <Link to={`/admin/user/${item?._id}`}><span className='text-lg text-green-600 cursor-pointer'><MdEdit/></span></Link>
                                <span className='text-lg text-red-600 cursor-pointer ml-5' onClick={()=>deleteUserHandler(item?._id)} ><MdDelete/></span>
                            </div>
                        </div>
                    ))
                }
        ...
        <>
        {
            allUsers.length > productsPerPage && <div className='w-full flex items-center justify-center my-10'>
            <button 
